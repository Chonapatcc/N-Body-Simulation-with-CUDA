...
void calculate_acceleration(Particle& p, Tree* tree, double& ax, double& ay, double& az, double &theta)
{
    ...
    if(tree->is_leaf == true && tree->has_particle == true && &(tree->particle) == &p) return;

    d = calculate_distance(p,tree);
    if((tree->size / d) < theta || tree->is_leaf == true)
    {
        ...
        double magnitude = G * root->mass * invDistCube;

        ax += dx * magnitude;
        ay += dy * magnitude;
        az += dz * magnitude;

        double force = magnitude * p.mass;
        p.force += force;
    }
    else
    {
        for(int i =0 ;i<8;i++)
        {
            calculate_acceleration(p, tree->children[i],ax,ay,az,theta);
        }
    }
...
{
    Tree* root = new Tree();
    build_tree(root,vector<Particle*>(particles.begin(), particles.end()));
    theta = 0.5;
    for(int i =0 ; i<n ; i++)
    {
