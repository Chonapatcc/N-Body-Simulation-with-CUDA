...
__global__ void nbodySharedKernel(const float4* posMass_in,
                                  const float4* vel_in,
                                  float4* posMass_out,
                                  float4* vel_out,
                                  int N,
    ...
    for (int tileBase = 0; tileBase < N; tileBase += blockDim.x) {
        ...
        tilePosMass[tid] = jGlobal < N ? posMass_in[jGlobal]
        ...
        for (int j = 0; j < tileCount; ++j) {
            ...
            float force_scalar = G_CONST * mass * other.w * inv_dist3;
...
int main() {
    ...
    for (const std::string& filename : csvFiles) {
        ...
        std::cout << "Starting GPU N-Body simulation (shared-memory tiling)..." << std::endl;
        ...
        for (int step = 0; step < STEPS; ++step) {
            nbodySharedKernel<<<blocks, threadsPerBlock, sharedBytes>>>(
                d_posMass[current],
                d_vel[current],
                d_posMass[next],
                d_vel[next],
